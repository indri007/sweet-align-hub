...
Processes jobs.jsonl into:
  ...
  2. Vector store (ChromaDB or Qdrant) — text documents for RAG/semantic search
...
def setup_vector_store(jobs_data: list[dict]) -> int:
    """Insert job documents into the configured vector store. Returns document count."""
    config.ensure_data_dir()
    ...
    print(f"  Embedding and inserting {len(documents)} documents into vector store...")
