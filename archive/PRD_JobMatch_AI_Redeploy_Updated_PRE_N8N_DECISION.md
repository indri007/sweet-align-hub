...
**Service live**: `job-search-app` — project GCP `braided-trees-502809-r5`
...
### 2.1 Arsitektur saat ini (live di Platform Server)

```
Browser User
      │
      ▼
Streamlit App (Platform, job-search-app)
      │
...
| Hosting | Google Cloud, region `asia-southeast2` |
...
# Login & set project
gcloud auth login
gcloud config set project braided-trees-502809-r5
```
...
Buat file `.env` lokal (untuk testing di laptop, bukan untuk Platform Server):
...
## 8. Redeploy ke Platform Server dari Laptop Baru
...
### 9.2 Update Platform untuk pakai N8N
...
*Dokumen ini disusun berdasarkan riwayat kerja dan konfigurasi yang sudah diverifikasi langsung dari Cloud Shell (`braided-trees-502809` & `braided-trees-502809-r5`) serta dokumen rubrik resmi Final Project JCAI — Purwadhika.*
