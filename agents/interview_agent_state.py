...
def build_agent5_system_prompt(session: InterviewSession, jawaban_kandidat: Optional[str] = None) -> str:
    ...
    return (
        ...
        f"mewawancarai kandidat untuk posisi: {session.posisi}.\n"
        f"Progres wawancara: sudah {jumlah_pertanyaan} pertanyaan diajukan, topik yang sudah\n"
        f"dibahas: {', '.join(daftar_topik) if daftar_topik else '(belum ada)'}.\n\n"
